...
```{r}
tbl_posterior <- sample_posterior(mdl, tbl_mstate_interim, refresh = 100, nsim = 1000, warmup = 250, seed = 76947)

plot(mdl, dt = c(0, 36), sample = tbl_posterior)

tbl_posterior %>% 
  filter(parameter == "p") %>% 
  group_by(group_id) %>% 
  summarize(p_posterior_mean = mean(value))
```
