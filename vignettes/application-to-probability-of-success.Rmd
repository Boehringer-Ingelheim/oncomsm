...
pos <- c(
  A = mean(purrr::map_lgl(res, ~.$success[1])),
  B = mean(purrr::map_lgl(res, ~.$success[2])),
  C = mean(purrr::map_lgl(res, ~.$success[3]))
)
...
```{r, warning=FALSE}
recruitment_rates <- c(1, 1, 1)
tbl_example <- generate_visit_data(mdl, c(40, 40, 40), recruitment_rates) %>% 
