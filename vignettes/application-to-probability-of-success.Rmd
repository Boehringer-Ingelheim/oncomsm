...
Load the example interim data set. See `?tbl_interim_example` for details. 

```{r, eval=jags_found, warning=FALSE}
# sample from predictive and then reduce to t = 10
data("tbl_interim_example")
...
```{r}
tbl_interim_sample_sizes <- tbl_interim_example %>%
  select(group_id, subject_id) %>%
  distinct() %>%
  group_by(group_id) %>%
  summarize(n = n())

tbl_interim_sample_sizes
```
...
Combine observed data with 

```{r}
tbl_mstate_interim_forward <- bind_rows(
    tbl_interim_example, 
    tbl_to_be_recruited
  )
```
...
tbl_pos <- tbl_posterior_predictive %>%
    ungroup() %>%
    mutate(
      res = furrr::future_map(
        data, eval_phi_3,
