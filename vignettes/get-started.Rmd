...
enough to justify the assumption that non-respodners are unlikely to
...
way. This effectively means that the shape parameter has to be fixed *a
priori*.

```{r prior-specification}
mdl <- new_IndependentMixtureCureRateModel(
  group_id = c("A", "B", "C"),
  ...
  median_time_to_response_mean = c(4, 4, 4),
  median_time_to_response_sd = c(.25, .25, .25),
  max_time_to_response = c(18, 18, 18),
...
tmp <- sample_prior(mdl, rstan_output = TRUE, pars = c("p", "shape", "scale", "monthly_rate"))

bhmbasket.predict:::.impute.IndependentMixtureCureRateModel(
  mdl, 
  10, 
  data = tbl_visits %>% visits_to_tte(cutoff = 10)
)

bhmbasket.predict:::.impute.IndependentMixtureCureRateModel(
  mdl, 
  10, 
  data = tbl_visits %>% visits_to_tte()
)

rstan::extract(tmp, "p")[[1]]
rstan::extract(tmp, "shape")[[1]]
rstan::extract(tmp, "scale")[[1]]
rstan::extract(tmp, "monthly_rate")[[1]]
```
...
specified success threshold to each simulated trial.

```{r compute-planning-pos}
n_sim <- 250L # only for speed reasons - increase!

# sample from the prior predictive distribution
tbl_prior_predictive_samples <- draw_samples(mdl, n = n_per_arm, 
                                             seed = 42L, nsim = n_sim)
...
mdl$tte_model$logodds_min <- logodds(as.numeric(p_target))

# sample and plot again
plot(mdl, n = n_per_arm)
```

The corresponding success probability is the expected power.

```{r compute-planning-expected-power}
tbl_prior_predictive_samples <- draw_samples(mdl, n = n_per_arm, seed = 42L, nsim = n_sim)
...
params <- bhmbasket::setPriorParametersExNex(
  mu_mean   = logit(0.3), # exchangeable component mean normal prior mean
...
Looking at a snapshot of the visit data from 10 months into the trial
...
tbl_posterior_predictive_summary <- draw_samples(
    mdl, data = tbl_tte_interim, seed = 42L, nsim = n_sim,
    control = list(stepsize = 1e-4, adapt_delta = .9)
  ) %>% 
  group_by(iter, group_id) %>%
  tidyr::nest() %>%
  arrange(iter) %>% 
