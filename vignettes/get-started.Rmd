...
m <- 100L
tbl_prior_predictive_smpl <- draw_samples(model, n_to_be_recruited = 100L, nsim = m, seed = 42L)

plt <- survfit(Surv(t, is.finite(t)) ~ iter, data = tbl_prior_predictive_smpl) %>% 
...
tbl_prior_predictive2_smpl <- this_function_needs_a_good_name(
  ...
  draw_samples(n_to_be_recruited = 100L, nsim = m, seed = 42L)

plt2 <- survfit(Surv(t, is.finite(t)) ~ iter, data = tbl_prior_predictive2_smpl) %>% 
  ggsurvplot(data = tbl_prior_predictive2_smpl, palette = rep("black", m), alpha = .2)
...
m <- 100L
tbl_prior_predictive_smpl <- draw_samples(model, n_to_be_recruited = 20L, nsim = m, seed = 42L)
...
To calculate a probability of success from these predictions, the stopping 
criterion must be defined and, if it depends on time as well, the recruitment 
times relative to FPI must be sampled.

Assume that the recruitment time follows a Weibull distribution with shape $2$
and scale $12$.
```{r sample-recruitment-times}
set.seed(42)
tbl_combined <- left_join(
    tbl_prior_predictive_smpl,
    expand.grid(
      id = 1:20,
      iter = 1:m
    ) %>% 
    group_by(iter) %>% 
    mutate(
      t_recruitment = sort(rweibull(20, shape = 2, scale = 12))
    ) %>% 
    ungroup(),
    by = c("id", "iter")
)
...
# observed responses
dbl_pr_responses <- tbl_combined %>% 
  group_by(iter) %>% 
  summarize(
    pr_response = sum(t <= max(t_recruitment) + 6),
    .groups = "drop"
  ) %>% 
  pull(pr_response)
```

Assuming that the individuals are sorted by time point of study inclusion,
termination at exactly 6 months after inclusion of the last subject,
and a critical value of more than $c=6$ responses at the final analysis,
the probability of success would be `r mean(dbl_pr_responses > 6)`.
...
tbl_data %>% 
  rename(
    `subject id` = id,
    `earliest possible response time point` = t1,
    `latest possibleresponse time point` = t2
  ) %>% 
  knitr::kable(
    caption = "Observed data at time point of prediction.")
```

Examples of the imputes/predicted values of interest are shown below.

```{r simulate-data}
tbl_smpl <- draw_samples(model, 0L, data = tbl_data, nsim = m, seed = 42L)
...
tbl_smpl %>% 
  rename(
    ...
    `earliest possible response time point` = t1,
    `latest possibleresponse time point` = t2,
    `actual response time (sampled)` = t
  ) %>% 
  filter(iter <= 2) %>% 
  knitr::kable(
...
plt <- survfit(Surv(t, is.finite(t)) ~ iter, data = tbl_smpl) %>% 
  ggsurvplot(data = tbl_smpl, palette = rep("black", m), alpha = .2)

plt$plot + theme(legend.position = "none")
```

Assuming the same recruitment time model as before, one can again compute a 
...
The probability of success is now `r mean(dbl_pr_responses > 6)`.
