...
mdl_tte <- independent_mixture_cure_rate_model(
  group_id = c("A", "B", "C"),
  logodds_mean = c(log(.4/.6), log(.2/.8), log(.2/.8)),
  logodds_sd = c(1, 1, 1),
  logodds_min = c(-Inf, -Inf, -Inf),
  logodds_max = c(log(.6/.4), log(.6/.4), log(.6/.4)),
  shape_mean = c(4, 4, 4),
  shape_sd = c(2, 2, 2),
  scale_mean = c(5, 5, 5),
  scale_sd = c(1, 1, 1),
...
mdl_recruitment <- independent_poisson_recruitment_model(
  group_id = c("A", "B", "C"),
  log_monthly_rate_mean = c(log(2), log(1), log(1)),
...
tbl_prior_predictive_samples %>% 
  ...
  summarize(
    n = n(),
    r = sum(t + dt2 < Inf), # a response is observed if the response time point if finite
    .groups = "drop"
  ) %>% 
  mutate(
    `response rate` = r/n,
...
at least as large as the target value (assume that it is also 0.3).
...
mdl$tte_model$logodds_min <- c(log(.3/.7), log(.3/.7), log(.3/.7))
...
mdl$tte_model$logodds_min <- c(-Inf, -Inf, -Inf)

# specify priors for bhmbasket analysis (should be consistent)
params <- bhmbasket::setPriorParametersExNex(
  mu_mean   = log(0.2/0.8), # exchangeable component mean normal prior mean
  ...
  mu_j      = c(log(.4/.6), log(.2/.8), log(.2/.8)), # individual components means
