...
model <- this_function_needs_a_good_name(
  logodds_loc = 0, logodds_scale = 0.3, 
...
tbl_prior_predictive_smpl <- draw_samples(
  model, 
  data = tibble(
      subject_id = as.character(1:100),
      dt1 = rep(NA_real_, 100),
      dt2 = rep(NA_real_, 100),
    ),
  nsim = m, 
  seed = 42L
)
...
tbl_prior_predictive2_smpl <- this_function_needs_a_good_name(
    logodds_loc = 0, logodds_scale = .001, 
...
plt_nodata <- plt$plot + theme(legend.position = "none")
plt_nodata
``` 
...
mdl_recruitment <- exponential_recruitment_model(
    theta_loc = log(0.5), 
    theta_scale = .5, 
    dt_max = 5
  )

tbl_prior_predictive_recruitment <- draw_samples(
    mdl_recruitment,
    data = tibble(
      subject_id = as.character(1:20),
      t = rep(NA_real_, 20)
...
plt <- survfit(
    Surv(t, is.finite(t)) ~ iter, 
    data = tbl_prior_predictive_recruitment
  ) %>% 
  ggsurvplot(
    data = tbl_prior_predictive_recruitment, 
    palette = rep("black", m), 
...
Putting both models together we can sample from the prior predictive 
distribution to get samples of when patients are recruited and how long
they have to wait for a response (if they experience it at all).

```{r}
mdl <- bhmbasket.predict::model(
  model,
  mdl_recruitment
)

tbl_prior_predictive <- draw_samples(
  mdl, data = tibble(
    subject_id = as.character(1:20),
    t = rep(NA_real_),
    dt1 = rep(NA_real_),
...
pos <- tbl_prior_predictive %>% 
  group_by(iter) %>% 
  summarize(
    responses = sum(dt2 + t <= max(t) + 3),
    .groups = "drop"
  ) %>% 
  pull(responses) %>% 
  {mean(. > 6)}
...
Assuming termination of the trial exactly 3 months after inclusion of the 
last subject, 
...
tbl_data %>%
  transmute(
    ...
    `recruitment time` = t,
    `earliest possible response time` = t + dt1,
    `latest possibleresponse time` = t + dt2
  ) %>% 
  knitr::kable(
...
```{r plot-dynamic-prediction, fig.width=8}
plt <- survfit(
    Surv(dt, is.finite(dt)) ~ iter, 
    data = tbl_smpl
  ) %>% 
...
plt_conditional <- plt$plot + theme(legend.position = "none") + labs(title = "Conditional")
plt_nodata + labs(title = "Prior-Predictive") + plt_conditional
```
