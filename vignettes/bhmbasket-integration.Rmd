...
rstan::extract(smpl_prior, "p")[[1]] %>% colMeans() %>% round(3)
rstan::extract(smpl_posterior, "p")[[1]] %>% colMeans() %>% round(3)
```

The posterior shifts under the extreme evidence.
...
A shorthand for the these sampling steps is available with 

```{r}
tbl_decisions <- simulate_decision_rule(mdl, c(40, 40, 40), go,
                                        data = tbl_interim, seed = 32487,
                                        nsim = 100)
tbl_decisions %>% 
  group_by(group_id) %>% 
  summarize(`Pr[go]` = mean(go))
```
