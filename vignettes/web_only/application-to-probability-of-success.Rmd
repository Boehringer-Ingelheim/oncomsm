...
mdl_point <- create_srp_model(
  group_id = c("A", "B"),
  logodds_mean = logodds(c(0.4, 0.4)), # chose conflicting ORR parameters
...
pos <- function(tbl, nsim = 50L) {
  tbl_posterior_predictive <- impute_trial(
...
ggplot(tbl_pos_over_time) +
  aes(t_interim) +
  geom_errorbar(aes(ymin = PoS - se, ymax = PoS + se, color = group_id), width = 1) +
