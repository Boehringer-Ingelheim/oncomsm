...
times.Note that covariate modelling is not yet implemented in `oncomsm`.
...
Any differentiable re-parameterization $g^{-1}:g(a,b)\mapsto (a,b)$ whose
Jacobian's absolute determinant is $1/(a,b)$ thus leads to an (improper)
uniform reference prior via pull-back of the reference prior.
...
For the response probability $p$, we suggest a mixture prior consisting of a
$\operatorname{Beta}(\alpha,\beta)$ component with weight $1-\eta$ and a 
uniform component, $\operatorname{Beta}(1,1)$, with weight $\eta$.



## Choice of hyperparameters

Instead of specifying mean and variance of the underlying normal distribution on
the log median and log shape directly,
the $0.05$ and $0.95$ quantiles of the median and shape can be elicited to cover
...
If, for instance, a typical time from stable to response is 2 to 6 months
the corresponding log-normal prior for the median time-to-response would be:
...
The choice of the prior on the shape is less obvious.
In absence of evidence against constant transition hazards,
a stronger regularization towards a shape of $1$ makes sense to avoid
convergence issues.
It is suggested that the degree of regularization can be agreed on by inspecting
samples of potential survival curves under a fixed median survival time (here 6).
A sensible default choice seems to be to chose the quantiles $\pm 0.2$ around $\log(1.2)$
on the log scale, more variation allows extreme deviations in the shape that are 
less likely to occur in practice.

```{r}
res <- oncomsm:::get_mu_sigma(0.98, 1.46) # exp(log(1.2) - 0.2), exp(log(1.2) + 0.2)
...
Even with such a strong regularization, small samples might lead to a small 
number divergent transitions during sampling from the posterior.
This small number can be ignored or the prior can be chosen even narrower around a
supected shape paramters to avoid samplping issues completey.
...
set.seed(251L)
tibble(
  b = rlnorm(100, oncomsm:::get_mu_sigma(0.98, 1.46)$mu, oncomsm:::get_mu_sigma(0.98, 1.46)$sigma),
