...
is almost unidentifiable from only a few data points per transition.
To avoid convergence issues `oncomsm` uses a default of a point prior on $1$,
i.e. an exponential model.
If changes in the hazards over time are expected, it can make sense to
relax the prior at the risk of divergent transitions during sampling.
A small number of divergent transitions can usually be tolerated.
In practice, it is always possible to reduce the prior variability post-hoc or
