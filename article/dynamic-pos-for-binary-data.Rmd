...
Assumed that for each individual $i$ the visit date and status 
("SOT" = start of treatment) as well a whether or not the treatment continues 
is recorded. 

```{r example-recist-data}
tbl_recist_data <- readr::read_delim(
    file = system.file(package = "bhmbasket.predict", "example_recist_data.csv"),
    delim = ";",
    col_types = readr::cols(
      subject_id = readr::col_factor(),
      group_id = readr::col_factor(),
      visit_date = readr::col_date(format = "%d.%m.%Y"),
      visit_status = readr::col_factor(levels = c("SOT", "PD", "SD", "PR", "CR")),
      end_of_treatment = readr::col_logical()
    ) 
  ) %>% 
  mutate(
...
tbl_recist_data %>% 
  knitr::kable(
    caption = "Example RECIST data set."
  )
...
tbl_recist_data %>% 
  ggplot() +
    ...
    geom_point(
      aes(x = visit_t, y = subject_id, color = visit_status, shape = end_of_treatment), 
      size = 5
    ) +
    labs(
